...
/* ── Decorative visuals for each card ── */
  ...
  return (
    <div className="relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-b from-emerald-500/[0.04] to-transparent">
      ...
      <div className="flex w-full flex-col gap-3 px-8">
        {[
          { color: "bg-emerald-500/30", width: "w-[85%]", delay: "0s" },
          { color: "bg-teal-500/20", width: "w-[70%]", delay: "0.8s" },
          { color: "bg-emerald-400/25", width: "w-[92%]", delay: "1.6s" },
          { color: "bg-cyan-500/15", width: "w-[60%]", delay: "2.4s" },
          { color: "bg-teal-400/20", width: "w-[78%]", delay: "3.2s" },
        ].map((line, i) => (
          <div key={i} className={`h-[2px] ${line.width} overflow-hidden rounded-full`}>
            <div
              className={`h-full w-1/3 ${line.color} animate-[streamLine_4s_ease-in-out_infinite] rounded-full blur-[1px]`}
  ...
  return (
    <div className="relative flex h-48 items-end justify-center gap-[6px] overflow-hidden bg-gradient-to-b from-emerald-500/[0.04] to-transparent px-8 pb-8 pt-12">
      {bars.map((height, i) => (
        <div
          key={i}
          className="flex-1 animate-[barGrow_1.2s_ease-out_forwards] rounded-t-sm bg-gradient-to-t from-emerald-500/40 to-emerald-500/10"
          style={{
            height: 0,
            maxHeight: `${height}%`,
...
const features = [
  {
    visual: DataStreams,
    title: "Контроль действий в реальном времени",
    description:
      "Система фиксирует, чем занимается сотрудник: программы, сайты, операции с данными. Выявляет неэффективность автоматически.",
  },
  {
    visual: BarChart,
    title: "Объективная оценка продуктивности",
    description:
      "Автоматическая оценка продуктивности сотрудников и отделов. Руководитель получает объективную картину — только данные.",
  },
  {
    visual: ShieldPulse,
    title: "Предотвращение утечек данных",
    description:
      "Отслеживание подозрительных действий с файлами и информацией. Снижение риска утечек и защита от внутренних угроз.",
  },
];
    ...
    <section id="features" ref={sectionRef} className="px-6 py-24 md:py-32">
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className="mx-auto mb-4 max-w-3xl text-center text-4xl font-bold leading-tight md:text-5xl">
          <span className="text-white">Всё, что нужно для контроля бизнеса.</span>
          <br />
          <span className="text-white/50">В одной системе.</span>
        </h2>

        {/* Subtitle */}
        <p className="mx-auto mb-16 max-w-2xl text-center text-lg text-white/40">
          DataLock собирает данные о сотрудниках, процессах и рисках в одном месте.
        </p>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {features.map((feature, i) => {
            ...
            return (
              <div
                key={feature.title}
                className={`group overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] transition-all duration-500 hover:-translate-y-0.5 hover:border-white/[0.14] ${
                  visible
